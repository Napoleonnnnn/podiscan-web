    ...
    <main className="min-h-screen bg-[var(--background)]">
      ...
      <header className="bg-[var(--card-bg)] border-b border-[var(--card-border)] sticky top-0 z-50">
        <div className="px-4 sm:px-6 lg:px-10 xl:px-16 py-3 lg:py-4 flex items-center gap-3">
          <Link
            href="/history"
            className="flex items-center justify-center w-8 h-8 rounded-lg border border-[var(--card-border)] hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
          ...
          <div className="flex flex-col items-center gap-3 lg:gap-4 lg:w-[360px] shrink-0">
            ...
            <div className="flex gap-3 sm:gap-4 flex-wrap justify-center lg:justify-start w-full">
              {[
                ...
                <span key={item.label} className="flex items-center gap-1.5 text-[10px] sm:text-[11px] lg:text-xs text-[var(--muted)]">
          ...
          <div className="flex flex-col gap-4 sm:gap-5 lg:gap-6 flex-1 min-w-0">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 sm:gap-3 lg:gap-4">
              <TempCard label="Jempol" value={sesi.suhu.jempol} anomali={sesi.anomali && sesi.titikAnomali === "jempol"} />
              <TempCard label="Metatarsal 1" value={sesi.suhu.metatarsal_1} anomali={sesi.anomali && sesi.titikAnomali === "metatarsal_1"} />
              <TempCard label="Metatarsal 2" value={sesi.suhu.metatarsal_2} anomali={sesi.anomali && sesi.titikAnomali === "metatarsal_2"} />
              <TempCard label="Metatarsal 3" value={sesi.suhu.metatarsal_3} anomali={sesi.anomali && sesi.titikAnomali === "metatarsal_3"} />
              <TempCard label="Metatarsal 4" value={sesi.suhu.metatarsal_4} anomali={sesi.anomali && sesi.titikAnomali === "metatarsal_4"} />
              <TempCard label="Tumit" value={sesi.suhu.tumit} anomali={sesi.anomali && sesi.titikAnomali === "tumit"} />
            </div>

            {/* Info row */}
            <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl px-4 sm:px-5 py-3 sm:py-4">
              <p className="text-[10px] sm:text-[11px] text-[var(--muted)] uppercase tracking-wider font-medium mb-2">
                Informasi Pengukuran
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  <p className="text-[10px] text-[var(--muted)]">Sample Valid</p>
                  <p className="text-sm font-mono font-semibold text-[var(--foreground)]">{sesi.sampleValid}/10</p>
                </div>
                <div>
                  <p className="text-[10px] text-[var(--muted)]">Titik Anomali</p>
                  <p className="text-sm font-mono font-semibold text-[var(--foreground)]">
                    {sesi.titikAnomali ? sesi.titikAnomali.replace("_", " ") : "—"}
                  </p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-[10px] text-[var(--muted)]">Threshold</p>
